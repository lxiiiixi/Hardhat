...
contract CustomERC20 is InitializableOwnable {
    ...
    function _transfer(
        ...
        // sender 不可能为 address(0) 因为不可能操作零地址
        ...
        uint256 burnAmount; // 如果没有初始化默认为0
        uint256 feeAmount;
        if (tradeBurnRatio > 0) {
    ...
    function abandonOwnership(address zeroAddress) external onlyOwner {
        ...
        // newOwner 也需要重置
        // 如果 transferOwnership 之后newOwner指定了人，但是又立刻 abandonOwnership，只要这个人执行了acclaim，ownership还是
        _OWNER_ = address(0);
    }
}
