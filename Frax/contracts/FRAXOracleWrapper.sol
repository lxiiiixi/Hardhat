...
contract FRAXOracleWrapper is Owned {
    ...
    constructor(
        ...
        // FRAX / ETH
        priceFeedFRAXETH = AggregatorV3Interface(
    ...
    function getFRAXPrice()
        public
        view
        returns (uint256 raw_price, uint256 precise_price)
    {
        (, int price, , , ) = priceFeedFRAXETH.latestRoundData();
        // E6
        raw_price = uint256(price).mul(PRICE_PRECISION).div(
            uint256(10) ** chainlink_frax_eth_decimals
        );

        // E12
        precise_price = uint256(price)
            .mul(PRICE_PRECISION)
            .mul(EXTRA_PRECISION)
            .div(uint256(10) ** chainlink_frax_eth_decimals);
    }
    ...
    function consult(
        ...
        return
            PRICE_PRECISION.mul(PRICE_PRECISION).mul(EXTRA_PRECISION).div(
                frax_precise_price
            );
    }
    ...
    function setChainlinkFRAXETHOracle(
        address _chainlink_frax_eth_oracle
