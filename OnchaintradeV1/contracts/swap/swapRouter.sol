...
contract SwapRouter {
    ...
    function swapOut(
        ...
        if (address(tokenIn) == address(WETH)) {
            ...
            // token -> router
            IERC20(address(tokenIn)).safeTransferFrom(
                to,
                address(this),
                amountInMax
            );
    ...
    function removeLiquidity(
        ...
        if (address(token) == address(WETH)) {
            // call swap
            (uint256 amount, uint256 amountOsd) = swap.removeLiquidity(
                token,
                liquidity,
                address(this),
                deadline
