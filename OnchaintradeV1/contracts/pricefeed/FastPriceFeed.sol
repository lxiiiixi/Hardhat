...
contract FastPriceFeed is IFastPriceFeed, Ownable {
    ...
    function setMaxDeviationBasisPoints(uint256 _maxDeviationBasisPoints) external onlyOwner {
        maxDeviationBasisPoints = _maxDeviationBasisPoints;
    }

    function setTokens(address[] memory _tokens, uint256[] memory _tokenPrecisions)
        external
        onlyOwner
    {
    ...
    function setPrice(address _token, uint256 _price, uint256 _timestamp) external onlyOwner {
    ...
    function getPrice(address _token, uint256 _refPrice) external view override returns (uint256) {
        ...
        uint256 maxPrice = (_refPrice * (BASIS_POINTS_DIVISOR + maxDeviationBasisPoints)) /
            BASIS_POINTS_DIVISOR;
        uint256 minPrice = (_refPrice * (BASIS_POINTS_DIVISOR - maxDeviationBasisPoints)) /
            BASIS_POINTS_DIVISOR;
    ...
    function getPlainPrice(address _token) external view override returns (uint256) {
        if (block.timestamp > lastUpdatedAt + priceDuration) {
    ...
    function _emitPriceEvent(
        ...
        IFastPriceEvent(_fastPriceEvent).emitPriceEvent(_token, _price, _timestamp);
    }
