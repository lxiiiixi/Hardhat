...
// 在当前合约中我们只会使用到 XVSStore 的 safeRewardTransfer 函数，用于安全的转移奖励代币，避免因为精度问题导致的奖励代币无法提取的问题。
interface IXVSStore {
...
contract XVSVault is XVSVaultStorage, ECDSA, AccessControlledV5 {
     ...
     *
     *   这个保险库不支持通货紧缩的代币——它预计转移的代币数量将等于实际存入的金额。
     *   实际上，这意味着该保险库不支持USDT和类似的代币（不提供这些保证）。
     *
     */
    function add(
        ...
        require(
            address(xvsStore) != address(0),
            "Store contract addres is empty"
        );
    ...
    function set(
        ...
        PoolInfo[] storage poolInfo = poolInfos[_rewardToken];
        totalAllocPoints[_rewardToken] = totalAllocPoints[_rewardToken]
    ...
    function setWithdrawalLockingPeriod(
        ...
        emit WithdrawalLockingPeriodUpdated(
            _rewardToken,
            _pid,
            oldPeriod,
            _newPeriod
        ...
        )
    {
        ...
        for (
            uint i = _requests.length;
            i > 0 && isUnlocked(_requests[i - 1]);
            --i
        ) {
            if (_requests[i - 1].afterUpgrade == 1) {
                afterUpgradeWithdrawalAmount = afterUpgradeWithdrawalAmount.add(
                    _requests[i - 1].amount
                );
            } else {
                beforeUpgradeWithdrawalAmount = beforeUpgradeWithdrawalAmount
                    .add(_requests[i - 1].amount);
            }
    ...
    function executeWithdrawal(
        ...
        PoolInfo storage pool = poolInfos[_rewardToken][_pid];
        UserInfo storage user = userInfos[_rewardToken][_pid][msg.sender];
        WithdrawalRequest[] storage requests = withdrawalRequests[_rewardToken][
            _pid
        ][msg.sender];
        ...
        ) = popEligibleWithdrawalRequests(user, requests);
        require(
            beforeUpgradeWithdrawalAmount > 0 ||
                afterUpgradeWithdrawalAmount > 0,
            "nothing to withdraw"
        );
        ...
        if (beforeUpgradeWithdrawalAmount > 0) {
            ...
            totalPendingWithdrawals[_rewardToken][
                _pid
            ] = totalPendingWithdrawals[_rewardToken][_pid].sub(
                afterUpgradeWithdrawalAmount
            );
            pool.token.safeTransfer(
                address(msg.sender),
    ...
    function requestWithdrawal(
        ...
        UserInfo storage user = userInfos[_rewardToken][_pid][msg.sender];
        require(
            user.amount >= user.pendingWithdrawals.add(_amount),
        ...
        (beforeUpgradeWithdrawalAmount, ) = getRequestedWithdrawalAmount(
            requests
        );
        require(
            beforeUpgradeWithdrawalAmount == 0,
        ...
        totalPendingWithdrawals[_rewardToken][_pid] = totalPendingWithdrawals[
            _rewardToken
        ][_pid].add(_amount);
        user.rewardDebt = user
            .amount
    ...
    function getStakeAmount(address account) internal view returns (uint96) {
        require(
            xvsAddress != address(0),
            "XVSVault::getStakeAmount: xvs address is not set"
        );
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(
                    srcRepOld,
                    amount,
                    "XVSVault::_moveVotes: vote amount underflows"
                );
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(
                    dstRepOld,
                    amount,
                    "XVSVault::_moveVotes: vote amount overflows"
                );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "XVSVault::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function getPriorVotes(
        ...
        require(
            ...
            "XVSVault::getPriorVotes: not yet determined"
        );
    ...
    function _become(IXVSVaultProxy xvsVaultProxy) external {
        require(
            msg.sender == xvsVaultProxy.admin(),
            "only proxy admin can change brains"
        );
        require(
            xvsVaultProxy._acceptImplementation() == 0,
