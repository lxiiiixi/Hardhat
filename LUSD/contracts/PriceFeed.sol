...
/**
https://github.com/liquity/dev#pricefeed-and-oracle
包含获取当前 ETH:USD 价格的功能，系统使用该价格计算抵押率。
需要最新 ETH:USD 价格数据的流动性函数根据需要通过核心 PriceFeed.sol 合约动态获取价格，使用 Chainlink ETH:USD 参考合约作为其主要和 Tellor 的 ETH:USD 价格馈送作为其次要（后备） ） 数据源。 
PriceFeed 是有状态的，即它根据合约的当前状态记录可能来自两个来源之一的最后好价格。
 */
