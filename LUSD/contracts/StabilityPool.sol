...
/**
 * 包含稳定池操作的功能：进行存款和提取复合存款以及累积的 ETH 和 LQTY 收益。持有 LUSD 稳定池存款，以及存款人从清算中获得的 ETH 收益。
 *
 * 稳定池持有由稳定池存款人存入的LUSD代币。
 * 当Trove被清算时，根据系统条件，一部分LUSD债务将通过稳定池中的LUSD抵消：即，抵消债务会消失，并且相应数量的稳定池中的LUSD代币会被销毁。
 * 因此，每个存款人在清算时都会收到相应的LUSD损失，损失的比例与其存款在总存款中所占份额成比例。此外，他们还会收到ETH收益，因为被清算的Trove的ETH抵押品将按比例分配给稳定存款人。
 * 每次清算都会消耗同样的比例存款：例如，将稳定池中的总LUSD减少40％的清算会使每笔存款减少40％。
 * 经历一系列清算的存款称为“复合存款”：每次清算都会减少存款，将其乘以范围为[0,1]的某个数。
 *
 * --- 实现 ---
 * 我们使用高度可扩展的方法来跟踪存款和ETH收益，具有O(1)的复杂度。
 * 当清算发生时，我们不会更新每个存款人的存款和ETH收益，而是简单地更新两个状态变量：一个乘积P和一个总和S。
 * 通过数学处理，我们可以将初始存款因子分解出来，并使用这两个变量P和S准确跟踪所有存款人的复合存款和随时间累积的ETH收益，当清算发生时，只需使用这两个变量。当存款人加入稳定池时，他们会获得最新的P和S的快照：分别为P_t和S_t。
 * 对于给定的存款d_t，比率P/P_t告诉我们自加入稳定池以来存款减少的因子，而公式 d_t*(S-S_t)/P_t 给出了存款的总累积ETH收益。
 * 每次清算都会更新乘积P和总和S。在一系列清算之后，可以使用初始存款、存款人的P和S快照以及最新的P和S值来计算复合存款和相应的ETH收益。
 * 存款人更新其存款（取款、充值）时，他们的累积ETH收益会支付，他们的新存款会记录下来（基于其最新的复合存款并由取款/充值修改），并且他们会收到最新P和S的快照。实质上，他们进行了一笔新的存款，覆盖了旧的存款。
 *
 */
