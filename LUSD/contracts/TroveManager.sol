...
/*
 * 包含清算和赎回功能。它向 LQTYStaking 合约发送赎回费用。
 * 还包含每个 Trove 的状态 —— 即 Trove 的抵押品和债务的记录。
 * TroveManager 不持有价值（即以太币/其他代币）。
 * TroveManager 函数调用各种池，告诉他们在必要时在池之间移动以太币/代币。
 */
...
contract TroveManager is LiquityBase, Ownable, CheckContract, ITroveManager {
    ...
    // 根据上次清算时记录的totalStakes和totalCollateral快照计算新的质押品价值。
    ...
    // 将所有者的地址推入Trove所有者列表，并在Trove结构中记录相应的数组索引。
